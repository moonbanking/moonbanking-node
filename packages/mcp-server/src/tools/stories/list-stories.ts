  ...
  inputSchema: {
    ...
    properties: {
      include: {
        type: 'string',
        description:
          'An optional  comma-separated list of fields to include in the response. Possible values: `bank`, `country`, `paginationTotal`',
      },
      limit: {
        type: 'integer',
...
export const handler = async (client: MoonBanking, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await maybeFilter(jq_filter, await client.stories.list(body)));
};
