...
rotate. npm is configured to trust `publish-npm.yml` in this repository and
issues a short-lived credential to that workflow over OIDC.

Two consequences worth knowing: renaming or moving `publish-npm.yml` breaks
...
npm's OIDC path fails quietly — it falls back to token authentication, which
reports `ENEEDAUTH` with no further detail. A filename mismatch against the
trusted publisher configuration is the usual cause.
